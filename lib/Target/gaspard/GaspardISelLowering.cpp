//===-- GaspardISelLowering.cpp - Gaspard DAG Lowering Implementation ---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the GaspardTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
#include "GaspardMachineFunctionInfo.h"
...
#define DEBUG_TYPE "Gaspard-lower"
...
static cl::opt<int> GaspardLowerConstantMulThreshold(
    "Gaspard-constant-mul-threshold", cl::Hidden,
...
GaspardTargetLowering::GaspardTargetLowering(const TargetMachine &TM,
                                         const GaspardSubtarget &STI)
    : TargetLowering(TM) {
  // Set up the register classes.
  addRegisterClass(MVT::i32, &Gaspard::GPRRegClass);

  // Compute derived properties from the register classes
  TRI = STI.getRegisterInfo();
  computeRegisterProperties(TRI);

  setStackPointerRegisterToSaveRestore(Gaspard::SP);
...
SDValue GaspardTargetLowering::LowerOperation(SDValue Op,
...
//===----------------------------------------------------------------------===//
//                       Gaspard Inline Assembly Support
//===----------------------------------------------------------------------===//

Register GaspardTargetLowering::getRegisterByName(
  ...
  Register Reg = StringSwitch<unsigned>(RegName)
                     ...
                     .Case("sp", Gaspard::SP)
                     .Case("fp", Gaspard::FP)
                     .Case("rr1", Gaspard::RR1)
                     .Case("r10", Gaspard::R10)
                     .Case("rr2", Gaspard::RR2)
                     .Case("r11", Gaspard::R11)
                     .Case("rca", Gaspard::RCA)
                     .Default(0);
...
GaspardTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1)
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &Gaspard::GPRRegClass);
    default:
      break;
    }
...
GaspardTargetLowering::getSingleConstraintMatchWeight(
...
void GaspardTargetLowering::LowerAsmOperandForConstraint(
...
#include "GaspardGenCallingConv.inc"

static unsigned NumFixedArgs;
static bool CC_Gaspard32_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (ValNo < NumFixedArgs) {
    return CC_Gaspard32(ValNo, ValVT, LocVT, LocInfo, ArgFlags, State);
  }
...
SDValue GaspardTargetLowering::LowerFormalArguments(
...
SDValue GaspardTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Gaspard target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue GaspardTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  GaspardMachineFunctionInfo *GaspardMFI = MF.getInfo<GaspardMachineFunctionInfo>();
  ...
  if (CallConv == CallingConv::Fast) {
    CCInfo.AnalyzeFormalArguments(Ins, CC_Gaspard32_Fast);
  } else {
    CCInfo.AnalyzeFormalArguments(Ins, CC_Gaspard32);
  }
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i32: {
        Register VReg = RegInfo.createVirtualRegister(&Gaspard::GPRRegClass);
  ...
  // The Gaspard ABI for returning structs by value requires that we copy
  // the sret argument into rv for the return. Save the argument into
  // a virtual register so that we can access it from the return points.
  if (MF.getFunction().hasStructRetAttr()) {
    unsigned Reg = GaspardMFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(getRegClassFor(MVT::i32));
      GaspardMFI->setSRetReturnReg(Reg);
    }
  ...
  if (IsVarArg) {
    ...
    GaspardMFI->setVarArgsFrameIndex(FI);
  }
...
SDValue
GaspardTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool IsVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Gaspard32);
  ...
  if (DAG.getMachineFunction().getFunction().hasStructRetAttr()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, Gaspard::RV, Val, Flag);
    Flag = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(Gaspard::RV, getPointerTy(DAG.getDataLayout())));
  }

  RetOps[0] = Chain; // Update chain

  unsigned Opc = GaspardISD::RET_FLAG;
  if (Flag.getNode())
...
SDValue GaspardTargetLowering::LowerCCCCallTo(
  ...
  if (NumFixedArgs)
    CCInfo.AnalyzeCallOperands(Outs, CC_Gaspard32_VarArg);
  else {
    if (CallConv == CallingConv::Fast)
      CCInfo.AnalyzeCallOperands(Outs, CC_Gaspard32_Fast);
    else
      CCInfo.AnalyzeCallOperands(Outs, CC_Gaspard32);
  }
  ...
  for (unsigned I = 0, J = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, Gaspard::SP,
  ...
  uint8_t OpFlag = GaspardII::MO_NO_FLAG;
  if (G) {
  ...
  Chain = DAG.getNode(GaspardISD::CALL, DL, NodeTys,
...
SDValue GaspardTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_Gaspard32);
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and Gaspard only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue GaspardTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Flag =
      DAG.getNode(GaspardISD::SET_FLAG, DL, MVT::Glue, LHS, RHS, TargetCC);

  return DAG.getNode(GaspardISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Flag);
}

SDValue GaspardTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > GaspardLowerConstantMulThreshold)
    return SDValue();
...
SDValue GaspardTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(GaspardISD::SETCC, DL, Op.getValueType(), TargetCC, Flag);
}

SDValue GaspardTargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(GaspardISD::SELECT_CC, DL, VTs, TrueV, FalseV, TargetCC,
                     Flag);
}

SDValue GaspardTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  GaspardMachineFunctionInfo *FuncInfo = MF.getInfo<GaspardMachineFunctionInfo>();
...
SDValue GaspardTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For Gaspard, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(GaspardISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue GaspardTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue GaspardTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  EVT VT = Op.getValueType();
  SDLoc DL(Op);
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), DL, Gaspard::FP, VT);
  unsigned Depth = cast<ConstantSDNode>(Op.getOperand(0))->getZExtValue();
  while (Depth--) {
...
const char *GaspardTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case GaspardISD::ADJDYNALLOC:
    return "GaspardISD::ADJDYNALLOC";
  case GaspardISD::RET_FLAG:
    return "GaspardISD::RET_FLAG";
  case GaspardISD::CALL:
    return "GaspardISD::CALL";
  case GaspardISD::SELECT_CC:
    return "GaspardISD::SELECT_CC";
  case GaspardISD::SETCC:
    return "GaspardISD::SETCC";
  case GaspardISD::SUBBF:
    return "GaspardISD::SUBBF";
  case GaspardISD::SET_FLAG:
    return "GaspardISD::SET_FLAG";
  case GaspardISD::BR_CC:
    return "GaspardISD::BR_CC";
  case GaspardISD::Wrapper:
    return "GaspardISD::Wrapper";
  case GaspardISD::HI:
    return "GaspardISD::HI";
  case GaspardISD::LO:
    return "GaspardISD::LO";
  case GaspardISD::SMALL:
    return "GaspardISD::SMALL";
  default:
    return nullptr;
  }
}

SDValue GaspardTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  const Constant *C = N->getConstVal();
  const GaspardTargetObjectFile *TLOF =
      static_cast<const GaspardTargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlign(), N->getOffset(), GaspardII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Gaspard::R0, MVT::i32),
                       DAG.getNode(GaspardISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = GaspardII::MO_ABS_HI;
    uint8_t OpFlagLo = GaspardII::MO_ABS_LO;
    ...
    Hi = DAG.getNode(GaspardISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(GaspardISD::LO, DL, MVT::i32, Lo);
    SDValue Result = DAG.getNode(ISD::OR, DL, MVT::i32, Hi, Lo);
    return Result;
  }
}

SDValue GaspardTargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (TLOF->isGlobalInSmallSection(GO, getTargetMachine())) {
    SDValue Small = DAG.getTargetGlobalAddress(
        GV, DL, getPointerTy(DAG.getDataLayout()), Offset, GaspardII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Gaspard::R0, MVT::i32),
                       DAG.getNode(GaspardISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue GaspardTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue GaspardTargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small) {
    SDValue Small = DAG.getTargetJumpTable(
        JT->getIndex(), getPointerTy(DAG.getDataLayout()), GaspardII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Gaspard::R0, MVT::i32),
                       DAG.getNode(GaspardISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue GaspardTargetLowering::LowerSHL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // Gaspard shifts of larger than register sizes are wrapped rather than
...
SDValue GaspardTargetLowering::LowerSRL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of Gaspard's shift behavior to avoid needing to
...
SDValue GaspardTargetLowering::PerformDAGCombine(SDNode *N,
...
void GaspardTargetLowering::computeKnownBitsForTargetNode(
