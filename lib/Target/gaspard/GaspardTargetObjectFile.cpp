...
static cl::opt<unsigned> SSThreshold(
    "Gaspard-ssection-threshold", cl::Hidden,
    cl::desc("Small data and bss section threshold size (default=0)"),
    cl::init(0));

void GaspardTargetObjectFile::Initialize(MCContext &Ctx,
...
// section.
bool GaspardTargetObjectFile::isGlobalInSmallSection(
...
// Return true if this global address should be placed into small data/bss
// section.
bool GaspardTargetObjectFile::isGlobalInSmallSection(const GlobalObject *GO,
                                                   const TargetMachine &TM,
...
// kind.
bool GaspardTargetObjectFile::isGlobalInSmallSectionImpl(
    const GlobalObject *GO, const TargetMachine &TM) const {
...
MCSection *GaspardTargetObjectFile::SelectSectionForGlobal(
...
bool GaspardTargetObjectFile::isConstantInSmallSection(const DataLayout &DL,
                                                     const Constant *CN) const {
  return isInSmallSection(DL.getTypeAllocSize(CN->getType()));
}

MCSection *GaspardTargetObjectFile::getSectionForConstant(
