//===-- GaspardISelDAGToDAG.cpp - A dag to dag inst selector for Gaspard ------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the Gaspard target.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "Gaspard-isel"
...
//===----------------------------------------------------------------------===//
// GaspardDAGToDAGISel - Gaspard specific code to select Gaspard machine
// instructions for SelectionDAG operations.
//===----------------------------------------------------------------------===//
namespace {

class GaspardDAGToDAGISel : public SelectionDAGISel {
public:
  explicit GaspardDAGToDAGISel(GaspardTargetMachine &TargetMachine)
  ...
  StringRef getPassName() const override {
    return "Gaspard DAG->DAG Pattern Instruction Selection";
  }
...
// Helper functions for ComplexPattern used on GaspardInstrInfo
// Used on Gaspard Load/Store instructions.
bool GaspardDAGToDAGISel::selectAddrSls(SDValue Addr, SDValue &Offset) {
  ...
  if (Addr.getOpcode() == ISD::OR &&
      Addr.getOperand(1).getOpcode() == GaspardISD::SMALL) {
...
bool GaspardDAGToDAGISel::selectAddrRiSpls(SDValue Addr, SDValue &Base,
  ...
  if (ConstantSDNode *CN = dyn_cast<ConstantSDNode>(Addr)) {
    if (RiMode) {
      ...
      if (isInt<16>(CN->getSExtValue())) {
        ...
        Offset = CurDAG->getTargetConstant(Imm, DL, CN->getValueType(0));
        Base = CurDAG->getRegister(Gaspard::R0, CN->getValueType(0));
...
bool GaspardDAGToDAGISel::selectAddrRi(SDValue Addr, SDValue &Base,
                                     SDValue &Offset, SDValue &AluOp) {
  return selectAddrRiSpls(Addr, Base, Offset, AluOp, /*RiMode=*/true);
}

bool GaspardDAGToDAGISel::selectAddrSpls(SDValue Addr, SDValue &Base,
                                       SDValue &Offset, SDValue &AluOp) {
  return selectAddrRiSpls(Addr, Base, Offset, AluOp, /*RiMode=*/false);
}

bool GaspardDAGToDAGISel::selectAddrRr(SDValue Addr, SDValue &R1, SDValue &R2,
                                     SDValue &AluOp) {
  // if Address is FI, get the TargetFrameIndex.
  ...
  ISD::NodeType AluOperator = static_cast<ISD::NodeType>(Addr.getOpcode());
  LPAC::AluCode AluCode = LPAC::isdToGaspardAluCode(AluOperator);
  if (AluCode != LPAC::UNKNOWN) {
    ...
    if (Addr.getOperand(0).getOpcode() == GaspardISD::HI ||
        Addr.getOperand(0).getOpcode() == GaspardISD::LO ||
        Addr.getOperand(0).getOpcode() == GaspardISD::SMALL ||
        Addr.getOperand(1).getOpcode() == GaspardISD::HI ||
        Addr.getOperand(1).getOpcode() == GaspardISD::LO ||
        Addr.getOperand(1).getOpcode() == GaspardISD::SMALL)
      return false;
...
bool GaspardDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
void GaspardDAGToDAGISel::Select(SDNode *Node) {
...
void GaspardDAGToDAGISel::selectFrameIndex(SDNode *Node) {
  SDLoc DL(Node);
  SDValue Imm = CurDAG->getTargetConstant(0, DL, MVT::i32);
  int FI = cast<FrameIndexSDNode>(Node)->getIndex();
  EVT VT = Node->getValueType(0);
  SDValue TFI = CurDAG->getTargetFrameIndex(FI, VT);
  unsigned Opc = Gaspard::ADD_I_LO;
  if (Node->hasOneUse()) {
...
FunctionPass *llvm::createGaspardISelDag(GaspardTargetMachine &TM) {
  return new GaspardDAGToDAGISel(TM);
}
