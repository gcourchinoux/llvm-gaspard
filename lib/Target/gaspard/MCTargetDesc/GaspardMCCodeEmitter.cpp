//===-- GaspardMCCodeEmitter.cpp - Convert Gaspard code to machine code -------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the GaspardMCCodeEmitter class.
//
//===----------------------------------------------------------------------===//
...
class GaspardMCCodeEmitter : public MCCodeEmitter {
public:
  GaspardMCCodeEmitter(const MCInstrInfo &MCII, MCContext &C) {}
  GaspardMCCodeEmitter(const GaspardMCCodeEmitter &) = delete;
  void operator=(const GaspardMCCodeEmitter &) = delete;
  ~GaspardMCCodeEmitter() override = default;
...
static Gaspard::Fixups FixupKind(const MCExpr *Expr) {
  if (isa<MCSymbolRefExpr>(Expr))
    return Gaspard::FIXUP_Gaspard_21;
  if (const GaspardMCExpr *McExpr = dyn_cast<GaspardMCExpr>(Expr)) {
    GaspardMCExpr::VariantKind ExprKind = McExpr->getKind();
    switch (ExprKind) {
    case GaspardMCExpr::VK_Gaspard_None:
      return Gaspard::FIXUP_Gaspard_21;
    case GaspardMCExpr::VK_Gaspard_ABS_HI:
      return Gaspard::FIXUP_Gaspard_HI16;
    case GaspardMCExpr::VK_Gaspard_ABS_LO:
      return Gaspard::FIXUP_Gaspard_LO16;
    }
  }
  return Gaspard::Fixups(0);
}

// getMachineOpValue - Return binary encoding of operand. If the machine
// operand requires relocation, record the relocation and return zero.
unsigned GaspardMCCodeEmitter::getMachineOpValue(
    const MCInst &Inst, const MCOperand &MCOp, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  if (MCOp.isReg())
    return getGaspardRegisterNumbering(MCOp.getReg());
  if (MCOp.isImm())
  ...
  assert(isa<GaspardMCExpr>(Expr) || Expr->getKind() == MCExpr::SymbolRef);
...
static unsigned adjustPqBits(const MCInst &Inst, unsigned Value,
  ...
  if (!LPAC::isPostOp(AluCode) &&
      ((Op2.isImm() && Op2.getImm() != 0) ||
       (Op2.isReg() && Op2.getReg() != Gaspard::R0) || (Op2.isExpr())))
    Value |= (1 << PBitShift);
  ...
  if (LPAC::modifiesOp(AluCode) && ((Op2.isImm() && Op2.getImm() != 0) ||
                                    (Op2.isReg() && Op2.getReg() != Gaspard::R0)))
    Value |= (1 << QBitShift);
...
unsigned
GaspardMCCodeEmitter::adjustPqBitsRmAndRrm(const MCInst &Inst, unsigned Value,
                                         const MCSubtargetInfo &STI) const {
  return adjustPqBits(Inst, Value, 17, 16);
}

unsigned
GaspardMCCodeEmitter::adjustPqBitsSpls(const MCInst &Inst, unsigned Value,
                                     const MCSubtargetInfo &STI) const {
  return adjustPqBits(Inst, Value, 11, 10);
}

void GaspardMCCodeEmitter::encodeInstruction(
...
// Encode Gaspard Memory Operand
unsigned GaspardMCCodeEmitter::getRiMemoryOpValue(
  ...
  Encoding = (getGaspardRegisterNumbering(Op1.getReg()) << 18);
  if (Op2.isImm()) {
...
unsigned GaspardMCCodeEmitter::getRrMemoryOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  assert(Op1.isReg() && "First operand is not register.");
  Encoding = (getGaspardRegisterNumbering(Op1.getReg()) << 15);
  assert(Op2.isReg() && "Second operand is not register.");
  Encoding |= (getGaspardRegisterNumbering(Op2.getReg()) << 10);

  assert(AluMCOp.isImm() && "Third operator is not immediate.");
  // Set BBB
  unsigned AluOp = AluMCOp.getImm();
  Encoding |= LPAC::encodeGaspardAluCode(AluOp) << 5;
  // Set P and Q
...
unsigned
GaspardMCCodeEmitter::getSplsOpValue(const MCInst &Inst, unsigned OpNo,
                                   SmallVectorImpl<MCFixup> &Fixups,
                                   const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  Encoding = (getGaspardRegisterNumbering(Op1.getReg()) << 12);
  if (Op2.isImm()) {
...
unsigned GaspardMCCodeEmitter::getBranchTargetOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  ...
  Fixups.push_back(MCFixup::create(
      0, MCOp.getExpr(), static_cast<MCFixupKind>(Gaspard::FIXUP_Gaspard_25)));

  return 0;
}

#include "GaspardGenMCCodeEmitter.inc"

} // end namespace llvm

llvm::MCCodeEmitter *
llvm::createGaspardMCCodeEmitter(const MCInstrInfo &InstrInfo,
                               const MCRegisterInfo & /*MRI*/,
                               MCContext &context) {
  return new GaspardMCCodeEmitter(InstrInfo, context);
}
