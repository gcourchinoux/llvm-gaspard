//===-- GaspardDelaySlotFiller.cpp - Gaspard delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("Gaspard-nop-delay-filler", cl::init(false),
                       cl::desc("Fill Gaspard delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "Gaspard Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const GaspardSubtarget &Subtarget = MF.getSubtarget<GaspardSubtarget>();
...
// createGaspardDelaySlotFillerPass - Returns a pass that fills in delay
// slots in Gaspard MachineFunctions
FunctionPass *
llvm::createGaspardDelaySlotFillerPass(const GaspardTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == Gaspard::RET) {
        ...
        assert(RI->getOpcode() == Gaspard::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Gaspard::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Gaspard::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == Gaspard::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Gaspard::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Gaspard::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(Gaspard::NOP));
        }
        ++FilledSlots;
      }
