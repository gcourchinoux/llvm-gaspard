//===-- GaspardAsmParser.cpp - Parse Gaspard assembly to MCInst instructions --===//
//
...
struct GaspardOperand;

class GaspardAsmParser : public MCTargetAsmParser {
  // Parse operands
  std::unique_ptr<GaspardOperand> parseRegister(bool RestoreOnFailure = false);

  std::unique_ptr<GaspardOperand> parseImmediate();

  std::unique_ptr<GaspardOperand> parseIdentifier();
...
#include "GaspardGenAsmMatcher.inc"
...
public:
  GaspardAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
// GaspardOperand - Instances of this class represented a parsed machine
// instruction
struct GaspardOperand : public MCParsedAsmOperand {
  enum KindTy {
    TOKEN,
    REGISTER,
    IMMEDIATE,
  ...
  explicit GaspardOperand(KindTy Kind) : MCParsedAsmOperand(), Kind(Kind) {}
...
public:
  ...
  bool isHiImm16() {
    ...
    if (const GaspardMCExpr *SymbolRefExpr = dyn_cast<GaspardMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_ABS_HI;

    // Binary expression
    if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Imm.Value))
      if (const GaspardMCExpr *SymbolRefExpr =
              dyn_cast<GaspardMCExpr>(BinaryExpr->getLHS()))
        return SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_ABS_HI;
  ...
  bool isLoImm16() {
    ...
    // Symbolic reference expression
    if (const GaspardMCExpr *SymbolRefExpr = dyn_cast<GaspardMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_ABS_LO;
  ...
  bool isLoImm21() {
    ...
    // Symbolic reference expression
    if (const GaspardMCExpr *SymbolRefExpr = dyn_cast<GaspardMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_None;
  ...
  void addLoImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      Inst.addOperand(
          MCOperand::createImm(static_cast<int32_t>(ConstExpr->getValue())));
    else if (isa<GaspardMCExpr>(getImm())) {
#ifndef NDEBUG
      const GaspardMCExpr *SymbolRefExpr = dyn_cast<GaspardMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
      const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(getImm());
      assert(BinaryExpr && isa<GaspardMCExpr>(BinaryExpr->getLHS()) &&
             cast<GaspardMCExpr>(BinaryExpr->getLHS())->getKind() ==
                 GaspardMCExpr::VK_Gaspard_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else
  ...
  void addHiImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      const GaspardMCExpr *SymbolRefExpr = dyn_cast<GaspardMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
      ...
      assert(BinaryExpr && isa<GaspardMCExpr>(BinaryExpr->getLHS()) &&
             cast<GaspardMCExpr>(BinaryExpr->getLHS())->getKind() ==
                 GaspardMCExpr::VK_Gaspard_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else
      assert(false && "Operand type not supported.");
  ...
  void addLoImm21Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      const GaspardMCExpr *SymbolRefExpr = dyn_cast<GaspardMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_None);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
  ...
  static std::unique_ptr<GaspardOperand> CreateToken(StringRef Str, SMLoc Start) {
    auto Op = std::make_unique<GaspardOperand>(TOKEN);
  ...
  static std::unique_ptr<GaspardOperand> createReg(unsigned RegNum, SMLoc Start,
                                                 SMLoc End) {
    auto Op = std::make_unique<GaspardOperand>(REGISTER);
  ...
  static std::unique_ptr<GaspardOperand> createImm(const MCExpr *Value,
                                                 SMLoc Start, SMLoc End) {
    auto Op = std::make_unique<GaspardOperand>(IMMEDIATE);
    Op->Imm.Value = Value;
    Op->StartLoc = Start;
    Op->EndLoc = End;
    return Op;
  ...
  static std::unique_ptr<GaspardOperand>
  MorphToMemImm(std::unique_ptr<GaspardOperand> Op) {
  ...
  static std::unique_ptr<GaspardOperand>
  MorphToMemRegReg(unsigned BaseReg, std::unique_ptr<GaspardOperand> Op,
                   unsigned AluOp) {
  ...
  static std::unique_ptr<GaspardOperand>
  MorphToMemRegImm(unsigned BaseReg, std::unique_ptr<GaspardOperand> Op,
                   unsigned AluOp) {
    const MCExpr *Imm = Op->getImm();
...
bool GaspardAsmParser::ParseDirective(AsmToken /*DirectiveId*/) { return true; }

bool GaspardAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
                                             OperandVector &Operands,
                                             MCStreamer &Out,
                                             uint64_t &ErrorInfo,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((GaspardOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
...
std::unique_ptr<GaspardOperand>
GaspardAsmParser::parseRegister(bool RestoreOnFailure) {
  ...
  if (Lexer.getKind() == AsmToken::Identifier) {
    ...
    return GaspardOperand::createReg(RegNum, Start, End);
  }
  if (PercentTok.hasValue() && RestoreOnFailure)
    Lexer.UnLex(PercentTok.getValue());
  return nullptr;
...
bool GaspardAsmParser::ParseRegister(unsigned &RegNum, SMLoc &StartLoc,
                                   SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<GaspardOperand> Op = parseRegister(/*RestoreOnFailure=*/false);
  if (Op != nullptr)
    RegNum = Op->getReg();
  return (Op == nullptr);
}

OperandMatchResultTy GaspardAsmParser::tryParseRegister(unsigned &RegNum,
                                                      SMLoc &StartLoc,
                                                      SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<GaspardOperand> Op = parseRegister(/*RestoreOnFailure=*/true);
  if (Op == nullptr)
...
std::unique_ptr<GaspardOperand> GaspardAsmParser::parseIdentifier() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  const MCExpr *Res, *RHS = nullptr;
  GaspardMCExpr::VariantKind Kind = GaspardMCExpr::VK_Gaspard_None;
  ...
  // identifier.
  if (Kind != GaspardMCExpr::VK_Gaspard_None) {
  ...
  Res = GaspardMCExpr::create(Kind, Expr, getContext());

  // Nest if this was an addition
  if (RHS)
    Res = MCBinaryExpr::createAdd(Res, RHS, getContext());

  return GaspardOperand::createImm(Res, Start, End);
}

std::unique_ptr<GaspardOperand> GaspardAsmParser::parseImmediate() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  ...
  case AsmToken::Dot:
    if (!Parser.parseExpression(ExprVal))
      return GaspardOperand::createImm(ExprVal, Start, End);
    LLVM_FALLTHROUGH;
  default:
    return nullptr;
  }
...
unsigned GaspardAsmParser::parseAluOperator(bool PreOp, bool PostOp) {
  StringRef IdString;
  Parser.parseIdentifier(IdString);
  unsigned AluCode = LPAC::stringToGaspardAluCode(IdString);
...
bool GaspardAsmParser::parsePrePost(StringRef Type, int *OffsetValue) {
...
bool shouldBeSls(const GaspardOperand &Op) {
  ...
  if (const GaspardMCExpr *SymbolRefExpr = dyn_cast<GaspardMCExpr>(Op.getImm()))
    return SymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_None;
  ...
  if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Op.getImm())) {
    const GaspardMCExpr *LHSSymbolRefExpr =
        dyn_cast<GaspardMCExpr>(BinaryExpr->getLHS());
    return (LHSSymbolRefExpr &&
            LHSSymbolRefExpr->getKind() == GaspardMCExpr::VK_Gaspard_None);
  }
  return false;
}
...
GaspardAsmParser::parseMemoryOperand(OperandVector &Operands) {
  ...
  if (Operands[0]->isToken())
    Type = static_cast<GaspardOperand *>(Operands[0].get())->getToken();
  ...
  std::unique_ptr<GaspardOperand> Op = parseRegister();
  if (!Op)
    Op = parseImmediate();
  ...
  Parser.Lex(); // Eat the '['.
  std::unique_ptr<GaspardOperand> Offset = nullptr;
  if (Op)
    Offset.swap(Op);
  ...
  if (!Op) {
    if (!Offset) {
      if ((Op = parseImmediate()) && Lexer.is(AsmToken::RBrac)) {
        ...
        // SLS, the rest as RM.
        if (shouldBeSls(*Op)) {
          Operands.push_back(GaspardOperand::MorphToMemImm(std::move(Op)));
        } else {
          ...
          Operands.push_back(GaspardOperand::MorphToMemRegImm(
              Gaspard::R0, std::move(Op), LPAC::ADD));
        }
        return MatchOperand_Success;
      }
    }
  ...
  if (Lexer.is(AsmToken::RBrac)) {
    ...
    if (!Offset) {
      ...
      Offset = GaspardOperand::createImm(OffsetConstExpr, Start, End);
    }
  } else {
  ...
  Operands.push_back(
      Offset->isImm()
          ? GaspardOperand::MorphToMemRegImm(BaseReg, std::move(Offset), AluOp)
          : GaspardOperand::MorphToMemRegReg(BaseReg, std::move(Offset), AluOp));
...
OperandMatchResultTy
GaspardAsmParser::parseOperand(OperandVector *Operands, StringRef Mnemonic) {
...
StringRef GaspardAsmParser::splitMnemonic(StringRef Name, SMLoc NameLoc,
                                        OperandVector *Operands) {
  ...
  if (Mnemonic[0] == 'b' ||
    ...
    LPCC::CondCode CondCode =
        LPCC::suffixToGaspardCondCode(Mnemonic.substr(1, Next));
    if (CondCode != LPCC::UNKNOWN) {
      Mnemonic = Mnemonic.slice(0, 1);
      Operands->push_back(GaspardOperand::CreateToken(Mnemonic, NameLoc));
      Operands->push_back(GaspardOperand::createImm(
          MCConstantExpr::create(CondCode, getContext()), NameLoc, NameLoc));
      if (IsBRR) {
        Operands->push_back(GaspardOperand::CreateToken(".r", NameLoc));
      }
      return Mnemonic;
    }
  }
  ...
  if (Mnemonic.startswith("sel") ||
    ...
    LPCC::CondCode CondCode = LPCC::suffixToGaspardCondCode(Mnemonic);
    if (CondCode != LPCC::UNKNOWN) {
...
static bool IsMemoryAssignmentError(const OperandVector &Operands) {
  ...
  if (GaspardOperand *PossibleAluOp =
          static_cast<GaspardOperand *>(Operands[PossibleAluOpIdx].get()))
...
static bool IsRegister(const MCParsedAsmOperand &op) {
  return static_cast<const GaspardOperand &>(op).isReg();
}
...
static bool MaybePredicatedInst(const OperandVector &Operands) {
  ...
  return StringSwitch<bool>(
             static_cast<const GaspardOperand &>(*Operands[0]).getToken())
...
bool GaspardAsmParser::ParseInstruction(ParseInstructionInfo & /*Info*/,
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name == "st" &&
    ...
    Operands.insert(Operands.begin(), GaspardOperand::CreateToken("s", NameLoc));
    Operands.insert(Operands.begin() + 1,
                    GaspardOperand::createImm(
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name.startswith("bt") &&
    ...
    Operands.insert(Operands.begin(), GaspardOperand::CreateToken("bt", NameLoc));
  }
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeGaspardAsmParser() {
  RegisterMCAsmParser<GaspardAsmParser> x(getTheGaspardTarget());
}
