//===-- GaspardFrameLowering.cpp - Gaspard Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Gaspard implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
void GaspardFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const GaspardRegisterInfo *LRI = STI.getRegisterInfo();
...
// ADJDYNALLOC pseudo instructions with a Gaspard:ADDI with the
// maximum call frame size as the immediate.
void GaspardFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  const GaspardInstrInfo &LII =
      *static_cast<const GaspardInstrInfo *>(STI.getInstrInfo());
  ...
  for (MachineFunction::iterator MBB = MF.begin(), E = MF.end(); MBB != E;
    ...
    while (MBBI != MBB->end()) {
      ...
      if (MI.getOpcode() == Gaspard::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        Register Dst = MI.getOperand(0).getReg();
        Register Src = MI.getOperand(1).getReg();

        BuildMI(*MBB, MI, DL, LII.get(Gaspard::ADD_I_LO), Dst)
            .addReg(Src)
...
void GaspardFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(Gaspard::SW_RI))
      .addReg(Gaspard::FP)
      .addReg(Gaspard::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(Gaspard::ADD_I_LO), Gaspard::FP)
      .addReg(Gaspard::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(Gaspard::SUB_I_LO), Gaspard::SP)
        .addReg(Gaspard::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
MachineBasicBlock::iterator GaspardFrameLowering::eliminateCallFramePseudoInstr(
...
void GaspardFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
                                      MachineBasicBlock &MBB) const {
  ...
  BuildMI(MBB, MBBI, DL, LII.get(Gaspard::ADD_I_LO), Gaspard::SP)
      .addReg(Gaspard::FP)
      .addImm(0);

  // Restore the frame pointer from the stack.
  BuildMI(MBB, MBBI, DL, LII.get(Gaspard::LDW_RI), Gaspard::FP)
      .addReg(Gaspard::FP)
      .addImm(-8)
      .addImm(LPAC::ADD);
}

void GaspardFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
                                              RegScavenger *RS) const {
  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);

  MachineFrameInfo &MFI = MF.getFrameInfo();
  const GaspardRegisterInfo *LRI =
      static_cast<const GaspardRegisterInfo *>(STI.getRegisterInfo());
  int Offset = -4;
