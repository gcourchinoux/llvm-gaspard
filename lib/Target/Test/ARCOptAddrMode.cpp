...
#define OPTADDRMODE_DESC "Test load/store address mode"
#define OPTADDRMODE_NAME "Test-addr-mode"
#define DEBUG_TYPE "Test-addr-mode"

namespace llvm {
FunctionPass *createTestOptAddrMode();
void initializeTestOptAddrModePass(PassRegistry &);
} // end namespace llvm

namespace {
class TestOptAddrMode : public MachineFunctionPass {
public:
  static char ID;

  TestOptAddrMode() : MachineFunctionPass(ID) {}
...
private:
  const TestSubtarget *AST = nullptr;
  const TestInstrInfo *AII = nullptr;
...
char TestOptAddrMode::ID = 0;
INITIALIZE_PASS_BEGIN(TestOptAddrMode, OPTADDRMODE_NAME, OPTADDRMODE_DESC, false,
                      false)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_END(TestOptAddrMode, OPTADDRMODE_NAME, OPTADDRMODE_DESC, false,
                    false)
...
bool TestOptAddrMode::noUseOfAddBeforeLoadOrStore(const MachineInstr *Add,
                                                 const MachineInstr *Ldst) {
  Register R = Add->getOperand(0).getReg();
  return dominatesAllUsesOf(Ldst, R, MDT, MRI);
}

MachineInstr *TestOptAddrMode::tryToCombine(MachineInstr &Ldst) {
  ...
  for (auto &Add : MRI->use_nodbg_instructions(B)) {
    ...
    int NewOpcode = Test::getPostIncOpcode(Ldst.getOpcode());
...
MachineInstr *
TestOptAddrMode::canJoinInstructions(MachineInstr *Ldst, MachineInstr *Add,
                                    SmallVectorImpl<MachineInstr *> *Uses) {
...
bool TestOptAddrMode::canFixPastUses(const ArrayRef<MachineInstr *> &Uses,
                                    MachineOperand &Incr, unsigned BaseReg) {
...
void TestOptAddrMode::fixPastUses(ArrayRef<MachineInstr *> Uses,
...
bool TestOptAddrMode::canHoistLoadStoreTo(MachineInstr *Ldst, MachineInstr *To) {
...
bool TestOptAddrMode::canSinkLoadStoreTo(MachineInstr *Ldst, MachineInstr *To) {
...
void TestOptAddrMode::changeToAddrMode(MachineInstr &Ldst, unsigned NewOpcode,
                                      unsigned NewBase,
...
bool TestOptAddrMode::processBasicBlock(MachineBasicBlock &MBB) {
  bool Changed = false;
  for (auto MI = MBB.begin(), ME = MBB.end(); MI != ME; ++MI) {
    ...
    if (!MI->mayLoad() && !MI->mayStore())
      continue;
    if (Test::getPostIncOpcode(MI->getOpcode()) < 0)
      continue;
...
bool TestOptAddrMode::runOnMachineFunction(MachineFunction &MF) {
  if (skipFunction(MF.getFunction()))
    return false;

  AST = &MF.getSubtarget<TestSubtarget>();
...
FunctionPass *llvm::createTestOptAddrMode() { return new TestOptAddrMode(); }
