//===- TestFrameLowering.cpp - Test Frame Information -------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Test implementation of the TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "Test-frame-lowering"

using namespace llvm;

static cl::opt<bool>
    UseSaveRestoreFunclet("Test-save-restore-funclet", cl::Hidden,
                          cl::desc("Use Test callee save/restore functions"),
                          cl::init(true));
...
static void generateStackAdjustment(MachineBasicBlock &MBB,
                                    ...
                                    const TestInstrInfo &TII, DebugLoc dl,
  ...
  if (isUInt<6>(AbsAmount))
    AdjOp = Positive ? Test::ADD_rru6 : Test::SUB_rru6;
  else if (isInt<12>(AbsAmount))
    AdjOp = Positive ? Test::ADD_rrs12 : Test::SUB_rrs12;
  else
    AdjOp = Positive ? Test::ADD_rrlimm : Test::SUB_rrlimm;
...
static unsigned determineLastCalleeSave(ArrayRef<CalleeSavedInfo> CSI) {
  unsigned Last = 0;
  for (auto Reg : CSI) {
    assert(Reg.getReg() >= Test::R13 && Reg.getReg() <= Test::R25 &&
...
void TestFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                            BitVector &SavedRegs,
                                            RegScavenger *RS) const {
  LLVM_DEBUG(dbgs() << "Determine Callee Saves: " << MF.getName() << "\n");
  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);
  SavedRegs.set(Test::BLINK);
}

void TestFrameLowering::adjustStackToMatchRecords(
...
/// For Test, this inserts a call to a function that puts required callee saved
/// registers onto the stack, when enough callee saved registers are required.
void TestFrameLowering::emitPrologue(MachineFunction &MF,
                                    MachineBasicBlock &MBB) const {
  LLVM_DEBUG(dbgs() << "Emit Prologue: " << MF.getName() << "\n");
  auto *AFI = MF.getInfo<TestFunctionInfo>();
  ...
  if (MF.getFunction().isVarArg()) {
    ...
    unsigned Opc = Test::SUB_rrlimm;
    if (isUInt<6>(VarArgsBytes))
      Opc = Test::SUB_rru6;
    else if (isInt<12>(VarArgsBytes))
      Opc = Test::SUB_rrs12;
    BuildMI(MBB, MBBI, dl, TII->get(Opc), Test::SP)
        .addReg(Test::SP)
        .addImm(VarArgsBytes);
  }
  if (hasFP(MF)) {
    LLVM_DEBUG(dbgs() << "Saving FP\n");
    BuildMI(MBB, MBBI, dl, TII->get(Test::ST_AW_rs9))
        .addReg(Test::SP, RegState::Define)
        .addReg(Test::FP)
        .addReg(Test::SP)
        .addImm(-4);
    AlreadyAdjusted += 4;
  }
  if (UseSaveRestoreFunclet && Last > Test::R14) {
    LLVM_DEBUG(dbgs() << "Creating store funclet.\n");
    // BL to __save_r13_to_<TRI->getRegAsmName()>
    StackSlotsUsedByFunclet = Last - Test::R12;
    BuildMI(MBB, MBBI, dl, TII->get(Test::PUSH_S_BLINK));
    BuildMI(MBB, MBBI, dl, TII->get(Test::SUB_rru6))
        .addReg(Test::SP)
        .addReg(Test::SP)
        .addImm(4 * StackSlotsUsedByFunclet);
    BuildMI(MBB, MBBI, dl, TII->get(Test::BL))
        .addExternalSymbol(store_funclet_name[Last - Test::R15])
        .addReg(Test::BLINK, RegState::Implicit | RegState::Kill);
  ...
  generateStackAdjustment(MBB, MBBI, *ST.getInstrInfo(), dl,
                          -(MFI.getStackSize() - AlreadyAdjusted), Test::SP);

  if (hasFP(MF)) {
    LLVM_DEBUG(dbgs() << "Setting FP from SP.\n");
    BuildMI(MBB, MBBI, dl,
            TII->get(isUInt<6>(MFI.getStackSize()) ? Test::ADD_rru6
                                                   : Test::ADD_rrlimm),
            Test::FP)
        .addReg(Test::SP)
  ...
  if (hasFP(MF)) {
    CFIIndex = MF.addFrameInst(MCCFIInstruction::createOffset(
        nullptr, MRI->getDwarfRegNum(Test::FP, true), CurOffset));
    BuildMI(MBB, MBBI, dl, TII->get(TargetOpcode::CFI_INSTRUCTION))
        .addCFIIndex(CFIIndex)
        .setMIFlags(MachineInstr::FrameSetup);
    CurOffset -= 4;
  ...
  if (MFI.hasCalls()) {
    CFIIndex = MF.addFrameInst(MCCFIInstruction::createOffset(
        nullptr, MRI->getDwarfRegNum(Test::BLINK, true), CurOffset));
    BuildMI(MBB, MBBI, dl, TII->get(TargetOpcode::CFI_INSTRUCTION))
        .addCFIIndex(CFIIndex)
        .setMIFlags(MachineInstr::FrameSetup);
  }
  ...
  for (const auto &Entry : CSI) {
    ...
    // Skip BLINK and FP.
    if ((hasFP(MF) && Reg == Test::FP) || (MFI.hasCalls() && Reg == Test::BLINK))
      continue;
    CFIIndex = MF.addFrameInst(MCCFIInstruction::createOffset(
...
/// For Test, this inserts a call to a function that restores callee saved
/// registers onto the stack, when enough callee saved registers are required.
void TestFrameLowering::emitEpilogue(MachineFunction &MF,
                                    MachineBasicBlock &MBB) const {
  ...
  if (hasFP(MF)) {
    ...
    if (isUInt<6>(StackSize))
      Opc = Test::SUB_rru6;
    BuildMI(MBB, MBBI, DebugLoc(), TII->get(Opc), Test::SP)
        .addReg(Test::FP)
        .addImm(StackSize);
  ...
  if (unsigned MoveAmount = StackSize - AmountAboveFunclet) {
    unsigned Opc = Test::ADD_rrlimm;
    if (isUInt<6>(MoveAmount))
      Opc = Test::ADD_rru6;
    else if (isInt<12>(MoveAmount))
      Opc = Test::ADD_rrs12;
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(Opc), Test::SP)
        .addReg(Test::SP)
  ...
  if (StackSlotsUsedByFunclet) {
    ...
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(Test::BL))
        .addExternalSymbol(load_funclet_name[Last - Test::R15])
        .addReg(Test::BLINK, RegState::Implicit | RegState::Kill);
    unsigned Opc = Test::ADD_rrlimm;
  ...
  if (SavedBlink) {
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(Test::POP_S_BLINK));
  }
  // Now, pop fp if necessary.
  if (hasFP(MF)) {
    BuildMI(MBB, MBBI, MBB.findDebugLoc(MBBI), TII->get(Test::LD_AB_rs9))
        .addReg(Test::FP, RegState::Define)
        .addReg(Test::SP, RegState::Define)
        .addReg(Test::SP)
        .addImm(4);
  }
...
bool TestFrameLowering::assignCalleeSavedSpillSlots(
  ...
  if (MFI.hasCalls() || (UseSaveRestoreFunclet && Last > Test::R14)) {
  ...
  for (unsigned Which = Last; Which > Test::R12; Which--) {
...
bool TestFrameLowering::spillCalleeSavedRegisters(
...
bool TestFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
...
void TestFrameLowering::processFunctionBeforeFrameFinalized(
  ...
  const TargetRegisterClass *RC = &Test::GPR32RegClass;
...
static void emitRegUpdate(MachineBasicBlock &MBB,
  ...
  if (isUInt<6>(NumBytes))
    Opc = IsAdd ? Test::ADD_rru6 : Test::SUB_rru6;
  else if (isInt<12>(NumBytes))
    Opc = IsAdd ? Test::ADD_rrs12 : Test::SUB_rrs12;
  else
    Opc = IsAdd ? Test::ADD_rrlimm : Test::SUB_rrlimm;
...
MachineBasicBlock::iterator TestFrameLowering::eliminateCallFramePseudoInstr(
  ...
  auto *AFI = MF.getInfo<TestFunctionInfo>();
  if (!hasFP(MF)) {
    if (Amt > AFI->MaxCallStackReq && Old.getOpcode() == Test::ADJCALLSTACKDOWN)
      AFI->MaxCallStackReq = Amt;
  } else {
    if (Amt != 0) {
      assert((Old.getOpcode() == Test::ADJCALLSTACKDOWN ||
              Old.getOpcode() == Test::ADJCALLSTACKUP) &&
             "Unknown Frame Pseudo.");
      bool IsAdd = (Old.getOpcode() == Test::ADJCALLSTACKUP);
      emitRegUpdate(MBB, I, dl, Test::SP, Amt, IsAdd, TII);
    }
  }
  return MBB.erase(I);
}

bool TestFrameLowering::hasFP(const MachineFunction &MF) const {
  const TargetRegisterInfo *RegInfo = MF.getSubtarget().getRegisterInfo();
